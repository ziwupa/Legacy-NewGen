...
class Modules:
    ...
    async def _register_modules(
        ...
        for mod in modules:
            try:
                ...
                code = Path(mod).read_text()

                if origin != "<core>":
                    # Modules of other userbots (restored from a backup, for
                    # instance) are loaded straight from the filesystem, so
                    # they have to be adapted here as well as in `.loadmod`
                    from .compat import geek, hikka

                    code = geek.compat(code)
                    code = hikka.compat(code)

                spec = importlib.machinery.ModuleSpec(
                    module_name,
                    StringLoader(code, user_friendly_origin),
    ...
    def _detach_module(self, mod: Module, purpose: str):
        """
        Fully detaches a module: it is not enough to drop it from `self.modules`,
        because its commands, watchers and handlers stay registered and keep
        being called (with `client_ready` never having finished, which ends in
        `AttributeError`s) until the junk collector wakes up
        """
    ...
    async def send_ready_one(
        ...
        try:
            ...
            self._detach_module(mod, "self-unload")
            ...
            self._detach_module(mod, "failed client_ready")
            raise
