...
class TestMod(loader.Module):
    ...
    async def logs(
        ...
        other = (
            ...
            (
                " <a"
                f' href="https://github.com/ziwupa/Legacy-NewGen/commit/{ghash}">@{ghash[:8]}</a>'
                if ghash
                else ""
            ),
        )
    ...
    async def ping(self, message):
        ...
        text = self.config["ping_text"].format(
            ...
            ping_hint=((self.config["hint"]) if random.choice([0, 0, 1]) == 1 else ""),
        ...
        if self.config["media_quote"]:
            ...
            await utils.answer_file(message, banner_url, text)
        else:
