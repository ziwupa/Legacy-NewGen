...
# Prefix of usernames of bots, created by Legacy
BOT_USERNAME_PREFIX = "NewGen"
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("legacy.inline", "custom_bot", False):
                ...
                try:
                    ...
                    uid = utils.rand(6)
                    username = f"@{BOT_USERNAME_PREFIX}_{uid}_bot"
            else:
                uid = utils.rand(6)
                username = f"@{BOT_USERNAME_PREFIX}_{uid}_bot"

            for msg in [
                "🌙 Legacy newgen userbot"[:64],
                username,
                "/setuserpic",
                username,
            ]:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    ...
                    if self._db.get("legacy.inline", "custom_bot", False) and (
                        self._db.get("legacy.inline", "custom_bot", False) != btn_text
                    ):
                        continue

                    if not self._db.get("legacy.inline", "custom_bot", False) and not (
                        (match := BOT_BASE_PATTERN.fullmatch(btn_text))
                        and (
                            match.group(1) == BOT_USERNAME_PREFIX
                            # Bots, created before the prefix was introduced
                            or match.group(1) in main.LATIN_MOCK
                        )
                    ):
                        continue
